...
<?php
	...
	function is_bot(){
		...
		if(!empty($_SERVER['HTTP_USER_AGENT'])&&(preg_match('~alexa|baidu|crawler|google|msn|yahoo~i',$_SERVER['HTTP_USER_AGENT'])||preg_match('~bot($|[^a-z])~i',$_SERVER['HTTP_USER_AGENT']))&&self::$options['link']=true)
			$is_bot=true;
		else
			$is_bot=false;
	...
	static $credits=' Powered by <a href="http://bmicalculator.fit" title="BMI Calculator">Calculate Your BMI</a> ';
	function credits(){
